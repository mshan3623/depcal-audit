...
#   조정 연도 = 양도월(2030-12)이 속한 FY2030 (더존식 양도월 포함 — core는
#   disposal_date=양도월+1 컨벤션이라 조정을 FY2031에 붙여 라벨만 1년 차이,
#   상각액·분배 수치는 동일. 더존식 기준 정답은 2030이므로 vcore를 고정한다).
#
#   2026-09-05(감사 G10): 조정을 **별도 행으로 붙이지 않고 종료해 행에 병합**한다. 종료월
#   양도는 조정 연도가 종료해와 같은 FY2030이라, 따로 붙이면 한 회계연도에 행이 둘이 되어
#   `fiscal_year`가 비유일해진다 — `next(r for r in sch if r.fiscal_year == fy)`류 소비자가
#   첫 행만 보고 **양도를 통째로 놓친다**. 분배 수치(3,999,600 / 400 / 5,999,400 / 600)는
#   손계산 그대로이고 바뀐 것은 행 구조뿐이다.
GOLDEN_SL_TERMINAL_MONTH_PARTIAL = [
    ...
    # 종료해 = 자연완료 상각(1,999,000) + 양도분 안분 제거를 반영한 기말 잔액
    (2030, 12, 1_999_000, 5_999_400,       600),
]


def test_vcore_sl_terminal_month_partial_disposal_adjusts():
    """종료월 부분양도가 무흔적으로 사라지지 않는다(과거 조용한 무시).

    흔적은 별도 행이 아니라 **종료해 행의 기말 잔액**에 남는다 — 같은 FY에 행을 둘로
    쪼개면 소비자가 첫 행만 보고 양도를 놓치기 때문이다(감사 G10).
    """
    ...
    assert [r[:3] for r in got] == [r[:3] for r in natural]   # 상각액은 자연완료와 동일
    assert got != natural                             # 그러나 양도 흔적(기말 잔액)은 남아야 함
    assert got[-1][3] + got[-1][4] == 6_000_000       # 잔존 누계+비망가 = 원가−양도액

    fys = [r[0] for r in got]
    assert len(fys) == len(set(fys))                  # 회계연도 라벨 유일 — 소비자가 놓치지 않는다
    # 순진한 소비자(첫 행만 보는 next)도 양도 후 잔액을 본다
    row_2030 = next(r for r in got if r[0] == 2030)
    assert (row_2030[3], row_2030[4]) == (5_999_400, 600)
